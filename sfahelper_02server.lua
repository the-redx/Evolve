...
function cmd_contract(arg)
  if pInfo.settings.rank < 13 then dtext('Данная функция доступна Полковнику и выше') return end
  if #arg == 0 then
...
function cmd_checkrank(arg)
  ...
  if sInfo.server ~= "185.169.134.68:7777" then dtext('Данная команда не доступна для вашего сервера') return end
  if #arg == 0 then
    ...
    for i = #tempFiles.ranks, 1, -1 do
      ...
      if line.nick == arg or line.nick == string.gsub(arg, "_", " ") then
        ...
        if line.executor ~= nil and line.reason ~= nil then 
          dtext(("Повысил: %s | Причина: %s"):format(line.executor, line.reason))
        end
        return
      end  
    end
  ...
  local updatelink = 'https://docs.google.com/spreadsheets/d/1FRA41gyKdBx1Okf-3865Ev2BX3dHTT2OyZg_mKLU1gY/export?format=tsv&id=1FRA41gyKdBx1Okf-3865Ev2BX3dHTT2OyZg_mKLU1gY&gid=1436531329'
  ...
  httpRequest(updatelink, nil, function(response, code, headers, status)
    if response then
      ...
      for line in response:gmatch('[^\r\n]+') do
        local arr = string.split(u8:decode(line), "\t")
        tempFiles.ranks[#tempFiles.ranks + 1] = { nick = arr[2], rank1 = arr[3], rank2 = arr[4], date = arr[5], executor = arr[6], reason = arr[7] }
      end
...
function cmd_checkpriziv(arg)
  ...
  if tempFiles.prizivTime >= os.time() - 180 then
    ...
    for i = #tempFiles.priziv, 1, -1 do
      ...
      if line.nick == arg or line.nick == string.gsub(arg, "_", " ") then
        ...
        if line.comissar ~= nil and line.date ~= nil and line.reason ~= nil then 
          dtext(("С %s на %s ранг | Внёс: %s | Дата: %s | Причина: %s"):format(line.rank1, line.rank2, line.comissar, line.date, line.reason))
        end
        return
      end  
    end
  ...
  -- Файл не загружен, или прошло более 3-х минут с момента прошлого обновления
  local updatelink = 'https://docs.google.com/spreadsheets/d/1FRA41gyKdBx1Okf-3865Ev2BX3dHTT2OyZg_mKLU1gY/export?format=tsv&id=1FRA41gyKdBx1Okf-3865Ev2BX3dHTT2OyZg_mKLU1gY&gid=1527340219'
  ...
  httpRequest(updatelink, nil, function(response, code, headers, status)
    if response then
      ...
      for line in response:gmatch('[^\r\n]+') do
        local arr = string.split(u8:decode(line), "\t")
        tempFiles.priziv[#tempFiles.priziv + 1] = { nick = arr[1], rank1 = arr[2], rank2 = arr[3], comissar = arr[5], date = arr[4], reason = arr[6] }
      end
      logger.trace("Обработка ответа успешно завершена")
      asyncQueue = false
      -- Обновляем время, возвращаемся в функцию
...
function cmd_checkvig(arg)
  ...
  if tempFiles.vigTime >= os.time() - 180 then
    ...
    for i = 1, #tempFiles.vig do
      ...
      if line.nick == arg or line.nick == string.gsub(arg, "_", " ") then
        ...
        if line.executor ~= nil and line.reason ~= nil then
          dtext(("%d. Выдал: %s | Причина: %s"):format(count + 1, line.executor, line.reason))
        end
        if line.vid ~= nil and line.date ~= nil and line.time ~= nil then 
          dtext(("    Вид: %s | Дата: %s | Срок: %s"):format(line.vid, line.date, line.time))
        end
        count = count + 1
      end
    end
  ...
  -- Файл не загружен, или прошло более 3-х минут с момента прошлого обновления
  local updatelink = 'https://docs.google.com/spreadsheets/d/1FRA41gyKdBx1Okf-3865Ev2BX3dHTT2OyZg_mKLU1gY/export?format=tsv&id=1FRA41gyKdBx1Okf-3865Ev2BX3dHTT2OyZg_mKLU1gY&gid=1995689502'
  ...
  httpRequest(updatelink, nil, function(response, code, headers, status)
    if response then
      ...
      for line in response:gmatch('[^\r\n]+') do
        local arr = string.split(u8:decode(line), "\t")
        tempFiles.vig[#tempFiles.vig + 1] = { executor = arr[2], nick = arr[1], vid = arr[3], reason = arr[6], date = arr[4], time = arr[5] }
      end
      logger.trace("Обработка ответа успешно завершена")
      asyncQueue = false
      -- Обновляем время, возвращаемся в функцию
...
imgui_windows.addtable = function()
  ...
  imgui.Combo('##combo', data.combo.addtable, u8"Не выбрано\0Повышение\0Контракт\0Выговор\0Инвайт\0\0")
  imgui.Separator()
  if data.combo.addtable.v > 0 then
    imgui.InputText(u8 'Введите ID/ник игрока', data.addtable.nick)
  end
  if data.combo.addtable.v == 1 or data.combo.addtable.v == 2 or data.combo.addtable.v == 4 then
    imgui.InputText(u8 'С какого ранга', data.addtable.param1)
    imgui.InputText(u8 'На какой ранг', data.addtable.param2)
    imgui.InputText(u8 'Примечание', data.addtable.reason)
  elseif data.combo.addtable.v == 3 then
    imgui.Text(u8'Вид выговора')
    imgui.Combo('##combo2', data.combo.addvig, u8"Выговор на уровне взвода\0Выговор на уровне армии\0\0")
    imgui.InputText(u8 'Нарушение', data.addtable.reason)
  end
  if data.combo.addtable.v > 0 then
    if imgui.Button(u8'Отправить') then
      ...
      local vigtype = data.combo.addvig.v
      local pid = tonumber(nickname)
      if sInfo.playerid == pid or sInfo.nick == nickname then atext('Вы не можете внести себя в таблицу!') return end
      if pid ~= nil then
        if sampIsPlayerConnected(pid) then nickname = sampGetPlayerNickname(pid)
        else atext('Неверный ID игрока!') return end
      end
      ---------
      if data.combo.addtable.v == 1 or data.combo.addtable.v == 2 or data.combo.addtable.v == 4 then
        if nickname == "" or param1 == "" or param2 == "" or reason == "" then atext('Все поля должны быть заполнены!') return end
        if tonumber(param1) == nil or tonumber(param1) < 0 or tonumber(param1) >= 15 or tonumber(param2) == nil or tonumber(param2) < 0 or tonumber(param2) >= 15 then atext('Неверные параметры ранга!') return end
        -----
        if data.combo.addtable.v == 1 then
          atext(("Повышение: [Ник: %s] [С ранга: %s] [На ранг: %s] [Примечание: %s]"):format(nickname, param1, param2, reason))
          sendGoogleMessage("giverank", nickname, param1, param2, reason, os.time())
        elseif data.combo.addtable.v == 2 then
          atext(("Контракт: [Ник: %s] [С ранга: %s] [На ранг: %s] [Примечание: %s]"):format(nickname, param1, param2, reason))
          sendGoogleMessage("contract", nickname, param1, param2, reason, os.time())
        elseif data.combo.addtable.v == 4 then
          atext(("Инвайт: [Ник: %s] [С ранга: %s] [На ранг: %s] [Примечание: %s]"):format(nickname, param1, param2, reason))
          sendGoogleMessage("prizivnik", nickname, param1, param2, reason, os.time())
        end
      elseif data.combo.addtable.v == 3 then
        if reason == "" or (vigtype ~= 0 and vigtype ~= 1) then atext('Все поля должны быть заполнены!') return end
        -----
        atext(("Выговор: [Ник: %s] [Тип: %s] [Нарушение: %s]"):format(nickname, vigtype == 0 and "Выговор на уровне взвода" or "Выговор на уровне армии", reason))
        sendGoogleMessage("reprimand", nickname, vigtype, _, reason, os.time())
      end
    end
  end
end
...
function sendGoogleMessage(type, name, param1, param2, reason, time)
  ...
  date = ("%d/%d/%d"):format(date.day, date.month, date.year, date.hour, date.min, date.sec)
  -- Формируем ссылки
  local url = ("?type=%s&executor=%s&who=%s"):format(type, mynick, name)
  if type == "giverank" or type == "contract" or type == "prizivnik" then
    url = url..("&param1=%s&param2=%s&reason=%s&date=%s"):format(encodeURI(u8:encode(param1)), encodeURI(u8:encode(param2)), encodeURI(u8:encode(reason)), date)
  elseif type == "reprimand" then
    if param1 == 0 then param1 = "Выговор на уровне взвода"
    else param1 = "Выговор на уровне армии" end
    url = url..("&param1=%s&date=%s&reason=%s&param2=1"):format(encodeURI(u8:encode(param1)), date, encodeURI(u8:encode(reason)))
  else return end
  ...
  lua_thread.create(function()
    ...
    downloadUrlToFile("https://script.google.com/macros/s/AKfycby5kaEueUIqk6lPuheLYuUQRKs5oR2tax0w-ObkOu1Rb-Ew9jw/exec"..url, downloadpath, function(id, status, p1, p2) -- remove
